...
const Headers = ({ cartLength }) => {
  ...
  return (
    <AppBar position="static" style={{ background: "#1F2A40" }}>
      <Toolbar>
        <Box sx={{ flexGrow: 1 }}>
          <NavLink to="/" className="text-decoration-none text-light mx-2" style={{textDecoration:"none"}}>
        ...
        {user && (
          ...
          >
            <IoMdLogOut style={{ fontSize: "large" }} /> <Typography sx={{fontSize:"medium" , fontWeight:"bold"}} >Logout</Typography>
          </Button>
        )}
      </Toolbar>
    </AppBar>
